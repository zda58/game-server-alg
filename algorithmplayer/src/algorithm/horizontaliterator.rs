...
impl HorizontalIterator {
    pub fn new(
        ...
        if x_coord > 0 {
            ...
            let heat = board[y_coord as usize][(x_coord - 1) as usize]
                .borrow()
                .heat;
                ...
                if self.shot_coords.borrow().contains(coord) {
                    if self.hit_coords.borrow().contains(coord) {
                        ...
                        if coord_x > 0 {
                            ...
                            let heat = self.board[coord_y as usize][(coord_x - 1) as usize]
                                .borrow()
                                .heat;
    ...
    pub fn is_coord_close(&self, coord: Coord) -> bool {
        ...
        if self.coord.borrow().y == coord_y {
            match &self.left {
                Some(coord) => {
                    if (coord.x as i32 - coord_x as i32) < 3
                        && (coord.x as i32 - coord_x as i32) > 0
                    {
                        return true;
                    }
                }
            ...
            match &self.right {
                Some(coord) => {
                    if (coord_x as i32 - coord.x as i32) < 3
                        && (coord_x as i32 - coord.x as i32) > 0
                    {
                        return true;
                    }
                }
